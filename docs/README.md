---
home: true
title: 咕咚文档
heroImage: /img/gudong-doc-logo.svg
footer: © 咕咚
sidebar: false
---

欢迎来到我的个人软件文档中心，选择上方入口开始使用。

<div class="app-grid">
  <a class="app-card" href="/inbox/">
    <img src="https://gudong.s3.bitiful.net/icon/inbox_ic_launcher.png" alt="inBox">
    <div class="app-name">inBox 笔记</div>
    <div class="app-desc">记录灵感、想法的本地化笔记软件</div>
  </a>
  <a class="app-card" href="/light/">
    <img src="https://cdn.jsdelivr.net/gh/maoruibin/assets@master/2024/09/17/20240917112408747.png" alt="点亮">
    <div class="app-name">点亮</div>
    <div class="app-desc">成为更好的自己，极致简单的习惯打卡软件</div>
  </a>
  <a class="app-card" href="/cang/">
    <img src="https://gudong.s3.bitiful.net/icon/cang_launcher_icon.png?no-wait=on" alt="仓咚咚">
    <div class="app-name">仓咚咚</div>
    <div class="app-desc">让资产一目了然，简单易用的资产管理软件</div>
  </a>
  <a class="app-card" href="/rssplus/">
    <img src="https://gudong.s3.bitiful.net/icon/rss_ic_launcher.png" alt="咕咚订阅">
    <div class="app-name">咕咚订阅</div>
    <div class="app-desc">个性化我的信息源，简洁实用的 RSS 订阅软件</div>
  </a>
  <a class="app-card" href="/picplus/">
    <img src="https://gudong.s3.bitiful.net/icon/pic_ic_launcher.png" alt="咕咚云图">
    <div class="app-name">咕咚云图</div>
    <div class="app-desc">图床上传 APP，生成 Markdown 链接，兼容主流图床与 GitHub/码云</div>
  </a>
</div>
